...
def get_function_fromhist(hist, func_type="totfunc", norm_fact = 1., colors=None):
    ...
    if func_type == "totfunc":
        ...
        hist.SetFillColorAlpha(ROOT.kRed, 0.5)
        spline.SetFillColorAlpha(ROOT.kRed, 0.5)
...
def plot(infile_name, colors, pt_min, pt_max, cent_min, cent_max):
    ...
    lat.DrawLatex(0.15, 0.68, f"{pt_min} < #it{{p}}_{{T}} < {pt_max} GeV/#it{{c}}")
    lat.DrawLatex(0.15, 0.62, f"{cent_min} #kern[-0.7]{{#font[122]{{-}}}} {cent_max}% FT0M")
