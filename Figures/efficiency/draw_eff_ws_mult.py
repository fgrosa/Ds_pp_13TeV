...
leg_dplus = ROOT.TLegend(0.05, 0.3, 0.9, 0.45)
leg_dplus.SetBorderSize(0)
leg_dplus.SetFillStyle(0)
leg_dplus.SetTextSize(0.045)
leg_dplus.SetNColumns(2)
leg_dplus.SetColumnSeparation(0.02)
leg_dplus.SetMargin(0.15)
for i_pt, h_dplus in enumerate(h_effs_dplus_vs_cent):
    h_dplus.Draw("same p")
    leg_dplus.AddEntry(h_dplus, f"{pt_mins[i_pt]} < #it{{p}}_{{T}} < {pt_maxs[i_pt]} GeV/#it{{c}}", "pl")
leg_dplus.Draw()

latex_dplus = ROOT.TLatex()
latex_dplus.SetTextFont(42)
latex_dplus.SetTextSize(0.05)
latex_dplus.DrawLatex(0.3, 2.1e-3, "D^{+}#rightarrow#phi#pi^{+}#rightarrowK^{+}K^{#minus}#pi^{+} and charge conj.")
latex_dplus.DrawLatex(0.3, 1.5e-3, "Prompt D^{+}")
