...
def ProduceFigure(multiTrialDict, multiTrialCfg, ptMin, ptMax):
    ...
    hFrame = ROOT.gPad.DrawFrame(0, min(np.array(multiTrialDict["rawyieldsDs"]+multiTrialDict["rawyieldsDplus"])-np.array(multiTrialDict["rawyieldsDs_err"]+multiTrialDict["rawyieldsDplus_err"]))*0.75, len(multiTrialDict["convergedTrials"])*2 + 1,
                 max(np.array(multiTrialDict["rawyieldsDs"]+multiTrialDict["rawyieldsDplus"])+np.array(multiTrialDict["rawyieldsDs_err"]+multiTrialDict["rawyieldsDplus_err"]))*1.4, 
                 "Raw Yields;Trial;Raw yield")
    hFrame.GetYaxis().SetLabelSize(0.035)
    hFrame.GetYaxis().SetTitleSize(0.04)
    hFrame.GetYaxis().SetTitleOffset(1.2)
    hFrame.GetXaxis().SetLabelSize(0.035)
    hFrame.GetXaxis().SetTitleSize(0.04)
    legend1 = ROOT.TLegend(0.3, 0.75, 0.9, 0.85)
    ...
    hFrame2 = ROOT.gPad.DrawFrame(min(multiTrialDict["ratios"])*0.9, 0., max(multiTrialDict["ratios"])*1.3,hist1.GetMaximum()*1.2, "D_{s}^{+}/D^{+} Ratio;Ratio;Entries")
    ...
    vLine = ROOT.TLine(central_value, 0, central_value, hist1.GetMaximum()*1.2)
    ...
    ROOT.gPad.SetTopMargin(0.1)
    ROOT.gPad.SetBottomMargin(0.12)
    hFrame = ROOT.gPad.DrawFrame(0, 
        min(np.array(multiTrialDict["sigmasDs"]+multiTrialDict["sigmasDplus"])-np.array(multiTrialDict["sigmasDs_err"]+multiTrialDict["sigmasDplus_err"]))*1000*0.75, 
        len(multiTrialDict["convergedTrials"])*2 + 1, 
        max(np.array(multiTrialDict["sigmasDs"]+multiTrialDict["sigmasDplus"])+np.array(multiTrialDict["sigmasDs_err"]+multiTrialDict["sigmasDplus_err"]))*1000*1.2, 
        "Peak widths;Trial;Width (MeV/#it{c}^{2})")
    hFrame.GetYaxis().SetLabelSize(0.035)
    hFrame.GetYaxis().SetTitleSize(0.04)
    hFrame.GetYaxis().SetTitleOffset(1.2)
    hFrame.GetXaxis().SetLabelSize(0.035)
    ...
    ptText = ROOT.TLatex(0.55, 0.37, f'{ptMin:.1f} < #it{{p}}_{{T}} < {ptMax:.1f} Ge#kern[-0.03]{{V}}/#it{{c}}')
    ptText.SetNDC()
    ...
    canvas.SaveAs(f'/home/fchinu/Run3/Ds_pp_13TeV/Figures/Systematics/RawYield/AllPtBins/RawYieldSyst{ptMin}_{ptMax}.pdf')
    canvas.SaveAs(f'/home/fchinu/Run3/Ds_pp_13TeV/Figures/Systematics/RawYield/AllPtBins/RawYieldSyst{ptMin}_{ptMax}.png')
...
if __name__=='__main__':
    ...
    for ptMin, ptMax in zip(ptMins, ptMaxs):
        ...
        if ptMin < 5:
            ...
            with open('/home/fchinu/Run3/Ds_pp_13TeV/Systematics/RawYields/config_multi_trial_highpt.yml', 'r') as f:
                analysisConfig = yaml.load(f, Loader=yaml.FullLoader)
        else:
            with open('/home/fchinu/Run3/Ds_pp_13TeV/Systematics/RawYields/config_multi_trial_highestpt.yml', 'r') as f:
                analysisConfig = yaml.load(f, Loader=yaml.FullLoader)


        

        ProduceFigure(analysisResults, analysisConfig['multitrial'], ptMin, ptMax)
