...
def ParquetFromRoot(input_file_path, output_file_path, treename, query=None, n_parts=2):
    '''
    ...
    for idx, (FileNamePart, DirNamesPart, TreeNamePart) in enumerate(zip(split_list(FileName, n_parts), split_list(DirNames, n_parts), split_list(TreeName, n_parts))):
        ...
        for i in range(len(DirNamesPart)):
            ...
            if query is None:
                ...
                df_temp = LoadDfFromRootOrParquet(FileNamePart[i], [DirNamesPart[i]], TreeNamePart[i]).query(query)
...
if __name__ == "__main__":
    ...
    input_file_path = "/home/fchinu/Run3/Ds_pp_13TeV/Datasets/Ds_pp_run3_ml/Merged_LHC22o_Train130748_AO2D.root"
    output_file_path = "/home/fchinu/Run3/Ds_pp_13TeV/Datasets/Ds_pp_run3_ml/Merged_LHC22o_Train130748_AO2D"   #No extension
    ...
    if isMC:
        ...
        MergeDataframes([output_file_path + f"_{i}.parquet" for i in range(n_divisions)], output_file_path + ".parquet.gzip")
