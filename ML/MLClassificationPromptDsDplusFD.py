...
def data_prep(inputCfg, iBin, PtBin, OutPutDirPt, PromptDsDf, PromptDplusDf, FDDf, BkgDf): #pylint: disable=too-many-statements, too-many-branches
    '''
    function for data preparation
    '''
    nPromptDs = len(PromptDsDf)
    nPromptDplus = len(PromptDplusDf)
    nFD = len(FDDf)
    nBkg = len(BkgDf)
    if FDDf.empty:
        out = f'\n     Prompt Ds: {nPromptDs}\n Signal prompt Dplus: {nPromptDplus}\n     Bkg: {nBkg}'
    else:
        out = f'\n     Prompt Ds: {nPromptDs}\n Signal prompt Dplus: {nPromptDplus}\n      FD: {nFD}\n     Bkg: {nBkg}'
    ...
    if dataset_opt == 'equal':
        if FDDf.empty:
            nCandToKeep = min([nPromptDs, nPromptDplus, nBkg])
            out = 'signal'
            out2 = 'signal'
        else:
            nCandToKeep = min([nPromptDs, nPromptDplus, nFD, nBkg])
            out = 'prompt Ds, prompt Dplus, FD'
            out2 = 'prompt'
        ...
        TotDf = pd.concat([BkgDf.iloc[:nCandToKeep], PromptDsDf.iloc[:nCandToKeep], PromptDplusDf.iloc[:nCandToKeep], FDDf.iloc[:nCandToKeep]], sort=True)
        if FDDf.empty:
            LabelsArray = np.array([0]*nCandToKeep + [1]*nCandToKeep + [2]*nCandToKeep)
        else:
            LabelsArray = np.array([0]*nCandToKeep + [1]*nCandToKeep + [2]*nCandToKeep + [3]*nCandToKeep)
        if test_f < 1:
        ...
        PromptDfSelForEff = pd.concat([PromptDsDf.iloc[nCandToKeep:], PromptDplusDf.iloc[nCandToKeep:], TestSet[np.array(yTest) == 1], TestSet[np.array(yTest) == 2]], sort=False)
        if FDDf.empty:
            FDDfSelForEff = pd.DataFrame()
        else:
            FDDfSelForEff = pd.concat([FDDf.iloc[nCandToKeep:], TestSet[np.array(yTest) == 3]], sort=False)
        del TotDf
...
#
    ...
    LegLabels = [inputCfg['output']['leg_labels']['Bkg'],
                 inputCfg['output']['leg_labels']['PromptDs'],
                 inputCfg['output']['leg_labels']['PromptDplus']]
    ...
    OutputLabels = [inputCfg['output']['out_labels']['Bkg'],
                    inputCfg['output']['out_labels']['PromptDs'],
                    inputCfg['output']['out_labels']['PromptDplus']]
    if inputCfg['output']['out_labels']['FD'] is not None:
        OutputLabels.append(inputCfg['output']['out_labels']['FD'])
    ListDf = [BkgDf, PromptDsDf, PromptDplusDf] if FDDf.empty else [BkgDf, PromptDsDf, PromptDplusDf, FDDf]
...
def train_test(inputCfg, PtBin, OutPutDirPt, TrainTestData, iBin): #pylint: disable=too-many-statements, too-many-branches
    ...
    if inputCfg['ml']['hyper_par_opt']['do_hyp_opt']:
        ...
        if (inputCfg['ml']['hyper_par_opt']['timeout'] is None) and (inputCfg['ml']['hyper_par_opt']['n_trials'] is None):
...
def main(): #pylint: disable=too-many-statements
    ...
    PromptDsHandler = TreeHandler(inputCfg['input']['promptDs'], inputCfg['input']['treename'])
    PromptDplusHandler = TreeHandler(inputCfg['input']['promptDplus'], inputCfg['input']['treename'])
    ...
    PromptDsHandler.slice_data_frame('fPt', PtBins, True)
    PromptDplusHandler.slice_data_frame('fPt', PtBins, True)
    ...
    for iBin, PtBin in enumerate(PtBins):
        ...
        TrainTestData, PromptDfSelForEff, FDDfSelForEff = data_prep(inputCfg, iBin, PtBin, OutPutDirPt,
                                                                    PromptDsHandler.get_slice(iBin), 
                                                                    PromptDplusHandler.get_slice(iBin),
                                                                    FDDfPt,
