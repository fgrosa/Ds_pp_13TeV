...
# Load the data
TreeFileName = "/home/fchinu/Run3/Ds_pp_13TeV/PYTHIA_Simulations/LcToPKPiContamination/lc_to_pkpi_pythia8_seed42.root"

Tree = uproot.open(TreeFileName)["treeLc"]
...
for iPt, (ptMin, ptMax) in enumerate(zip(ptMins, ptMaxs)):
    df_pt = df.query(f"ptLc > {ptMin} and ptLc < {ptMax}")
...
for mass, pt in zip(df["massKaKaPi"], df["ptLc"]):
...
ThesisLabel = ROOT.TLatex(0.2, 0.8, "This Thesis")
ThesisLabel.SetNDC()
ThesisLabel.SetTextFont(42)
ThesisLabel.SetTextSize(0.08)
PYTHIALabel = ROOT.TLatex(0.2, 0.73, "PYTHIA 8 simulation")
PYTHIALabel.SetNDC()
PYTHIALabel.SetTextFont(42)
PYTHIALabel.SetTextSize(0.05)
EventsLabel = ROOT.TLatex(0.2, 0.66, "10^{5} generated L_{c}^{+}")
EventsLabel.SetNDC()
EventsLabel.SetTextFont(42)
EventsLabel.SetTextSize(0.05)
DecayLabel = ROOT.TLatex(0.2, 0.59, "L_{c}^{+} #rightarrow p K^{#font[122]{-}} #pi^{+}")
DecayLabel.SetNDC()
DecayLabel.SetTextFont(42)
DecayLabel.SetTextSize(0.05)
MisIDLabel = ROOT.TLatex(0.2, 0.52, "#pi^{+} mis-identified as K^{+}")
MisIDLabel.SetNDC()
...
for i, h in enumerate(histos):
    hFrame = canvas.cd(i+1).DrawFrame(1., 0, 2.2, 0.1, ";Mass (GeV/#it{c}^{2});Counts (a.u.)")
    ...
    ptLabels.append(ROOT.TLatex(0.2, 0.45, f"{ptMins[i]} < #it{{p}}_{{T}} (L_{{c}}^{{+}}) < {ptMaxs[i]} GeV/#it{{c}}"))
    ptLabels[-1].SetNDC()
...
canvas.SaveAs("/home/fchinu/Run3/Ds_pp_13TeV/PYTHIA_Simulations/LcToPKPiContamination/Lc_Corr_Bkg_simulation.png")
canvas.SaveAs("/home/fchinu/Run3/Ds_pp_13TeV/PYTHIA_Simulations/LcToPKPiContamination/Lc_Corr_Bkg_simulation.pdf")


outfileName = "/home/fchinu/Run3/Ds_pp_13TeV/PYTHIA_Simulations/LcToPKPiContamination/Lc_Corr_Bkg_simulation.root"
